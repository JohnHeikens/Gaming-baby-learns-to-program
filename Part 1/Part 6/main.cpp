...
void onFrame() {
	...
	fp weightsred[layercount] = 
	{
		100,50,25,12.5,6.25
	};
	static LayerNoiseSimplex rednoise = LayerNoiseSimplex(0, weightsred, layercount, 0, 255, 0.0025);
	fp weightsgreen[layercount] =
	{
		100,50,25,12.5,6.25
	};
	static LayerNoiseSimplex greennoise = LayerNoiseSimplex(1, weightsgreen, layercount, 0, 255, 0.0025);
	fp weightsblue[layercount] =
	{
		100,50,25,12.5,6.25
	};
	static LayerNoiseSimplex bluenoise = LayerNoiseSimplex(2, weightsblue, layercount, 0, 255, 0.0025);
	color* ptr = colors;
	
	const int by = -height / 2, ey = by + height;
	for (int j = by; j < ey; j++)
	{
		const int bx = -width / 2, ex = bx + width;
		//fp percentagey = j / (height - 1.0);
		for (int i = bx; i < ex; i++)
		{
			//fp percentagex = i / (width - 1.0);
			*ptr++ = color(rednoise.Evaluate(i,j), greennoise.Evaluate(i, j), bluenoise.Evaluate(i, j));
		}
	}
	rednoise.Frequency *= 0.95;
	greennoise.Frequency *= 0.95;
	bluenoise.Frequency *= 0.95;
